  ...
  return (
    <section className="w-full px-4 py-8 sm:px-6 md:px-10 md:pb-24 lg:px-16">
      <motion.p
        ...
        className="mx-auto max-w-2xl text-center text-lg leading-relaxed text-slate-600 md:text-xl"
      >
      ...
      <div className="mx-auto mt-14 grid max-w-5xl grid-cols-1 gap-10 lg:grid-cols-[1fr_1.4fr] lg:gap-16">
        ...
        >
          ...
          <h2 className="mt-3 font-outfit text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
            Sample Topics
          </h2>
        </motion.div>
        ...
        >
          {topics.map((topic, i) => (
            <motion.div
              key={topic}
              variants={item}
              className="group flex items-start gap-4 border-b border-slate-200 py-6 first:pt-0 sm:gap-6"
            >
              <span className="font-outfit text-2xl font-bold text-slate-200 transition-colors group-hover:text-amber-500 sm:text-3xl">
                {String(i + 1).padStart(2, "0")}
              </span>
              <p className="mt-1 text-lg font-medium text-slate-900 transition-transform group-hover:translate-x-1 sm:text-xl">
                {topic}
              </p>
            </motion.div>
          ))}
